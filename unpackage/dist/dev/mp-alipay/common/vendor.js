...
function validateProtocol(name, data2, protocol, onFail) {
  ...
  for (const key in protocol) {
    const errMsg = validateProp$1(key, data2[key], protocol[key], !hasOwn$1(data2, key));
...
function invokeSuccess(id2, name, res) {
  return invokeCallback(id2, extend$1(res || {}, { errMsg: name + ":ok" }));
}
function invokeFail(id2, name, errMsg, errRes) {
  return invokeCallback(id2, extend$1({ errMsg: name + ":fail" + (errMsg ? " " + errMsg : "") }, errRes));
}
...
function setStorageSync(key, data2) {
...
const request = {
  ...
  args(fromArgs) {
    ...
    return {
      ...
      data(data2) {
        if (my.canIUse("saveFileToDingTalk") && method.toUpperCase() === "POST" && headers["content-type"].indexOf("application/json") === 0 && isPlainObject$1(data2)) {
          return {
            name: "data",
...
const previewImage = {
  args(fromArgs, toArgs) {
    ...
    if (isNaN(currentIndex)) {
      if (fromArgs.current && isArray$1(fromArgs.urls)) {
...
function vFor(source2, renderItem) {
  ...
  if (isArray$1(source2) || isString$1(source2)) {
    ...
    for (let i = 0, l = source2.length; i < l; i++) {
      ret[i] = renderItem(source2[i], i, i);
    }
  } else if (typeof source2 === "number") {
    if (!Number.isInteger(source2)) {
      warn$1(`The v-for range expect an integer value but got ${source2}.`);
      return [];
    }
    ...
    if (source2[Symbol.iterator]) {
      ret = Array.from(source2, (item, i) => renderItem(item, i, i));
    } else {
...
function initAppLifecycle(appOptions, vm) {
  ...
  if (isFunction$1(appOptions.onShow) && my.onAppShow) {
    my.onAppShow((args) => {
      vm.$callHook("onShow", args);
    });
  }
  if (isFunction$1(appOptions.onHide) && my.onAppHide) {
    my.onAppHide((args) => {
...
function initSpecialMethods(mpInstance) {
  ...
  if (specialMethods) {
    specialMethods.forEach((method) => {
      if (isFunction$1(mpInstance.$vm[method])) {
